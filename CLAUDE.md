...
10. **`qs` moderate advisory accepted.** `npm audit` reports one moderate finding: a remotely-triggerable DoS in `qs.stringify` for null/undefined entries in comma-format arrays with `encodeValuesOnly`. The vulnerable `qs@6.15.1` is transitive through `@modelcontextprotocol/sdk@1.29.0` → `express@5.2.1` → `body-parser`. We don't call `qs.stringify` ourselves, and the vulnerable code path requires user-controlled input feeding into a server we don't run with that body format. The SDK is already on its latest release; clearing this requires upstream to bump express/body-parser. CI's `npm audit --audit-level=high` doesn't fail on it (it's moderate, not high), but the finding is intentionally visible. Re-evaluate when the SDK ships a release that updates the chain.
